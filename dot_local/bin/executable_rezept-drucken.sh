...
# (2cm Rand, Helvetica, 12pt) und leitet dieses an lpr weiter.
...
margin_left="20mm"
margin_bottom="0mm"
# font="libertine" # requires texlive-fontsextra
font="helvet"
fontsize="12pt"
...
"$pandoc" --pdf-engine="$pdf_engine" -V geometry:top="$margin_top" -V geometry:left="$margin_left" -V geometry:bottom="$margin_bottom" -V fontfamily="$font" -V fontsize="$fontsize" -s "$FILE" -t pdf | "$lpr" -P "$PRINTER" "$DUPLEX" "$COLOR"
